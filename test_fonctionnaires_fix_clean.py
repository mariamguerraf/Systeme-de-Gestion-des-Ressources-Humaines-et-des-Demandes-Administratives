...
def test_fonctionnaires_flow():
    ...
    test_email = f"testfonc.modification.{timestamp}@univ.dz"
    ...
    if response.status_code != 200:
        print(f"❌ Failed to retrieve fonctionnaires list: {response.text}")
        return False
    
    fonctionnaires_list = response.json()
    updated_fonctionnaire = None
    for fonc in fonctionnaires_list:
        if fonc["id"] == fonctionnaire_id:
            updated_fonctionnaire = fonc
            break
    
    if not updated_fonctionnaire:
        print(f"❌ Fonctionnaire with ID {fonctionnaire_id} not found in list")
        return False
